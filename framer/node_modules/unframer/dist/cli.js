...
import { componentNameToPath, dedent, isTruthy, logger, sleep, spinner, } from './utils.js';
...
cli.command('mcp login [url]', 'Login by pasting your Framer MCP URL. Get the URL from Framer MCP plugin (Cmd/Ctrl+K > "MCP"). After login, run "unframer mcp skill" to see all available commands then. This cli will let you control Framer MCP via cli commands like `unframer mcp getProjectXml`').action(async (url) => {
...
cli.command('mcp skill', 'Show detailed help for all MCP commands with their options.').action(() => {
    const config = loadConfig();
    if (!config.mcpUrl) {
        console.log('No MCP URL configured. Run "unframer mcp login" first to connect to a Framer project.');
        return;
    }
    const mcpCommands = cli.commands.filter((cmd) => cmd.name.startsWith('mcp ') && cmd.name !== 'mcp login' && cmd.name !== 'mcp skill');
