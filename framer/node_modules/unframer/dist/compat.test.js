...
describe('propCamelCase', () => {
    test('converts dashes to camelCase', () => {
        ...
        expect(propCamelCaseJustLikeFramer('foo-bar-baz')).toMatchInlineSnapshot(`"fooBarBaz"`);
        expect(propCamelCaseJustLikeFramer('Detail - 1')).toMatchInlineSnapshot(`"detail1"`);
    });
    test('converts example-variable_dashes etc XX something 11 to camelCase', () => {
        expect(propCamelCaseJustLikeFramer('example-variable_dashes etc XX something 11')).toBe(`exampleVariableDashesEtcXXSomething11`);
    });
    test('converts 0 starts with ZeroEtc to camelCase', () => {
        expect(propCamelCaseJustLikeFramer('0 starts with ZeroEtc')).toBe(`_0StartsWithZeroEtc`);
    });
    test('converts underscores to camelCase', () => {
        expect(propCamelCaseJustLikeFramer('foo_bar')).toMatchInlineSnapshot(`"fooBar"`);
        expect(propCamelCaseJustLikeFramer('foo_bar_baz')).toMatchInlineSnapshot(`"fooBarBaz"`);
    });
    ...
    test('handles mixed cases', () => {
        expect(propCamelCaseJustLikeFramer('Foo-Bar_Baz Test')).toMatchInlineSnapshot(`"fooBar_BazTest"`);
    });
