...
test('* param in .route() does not contain leading slash', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/repos/user/myrepo/main/file/src/components/Button.tsx', {
        method: 'GET',
    }));
    expect(res.status).toBe(200);
...
test('* method listens on all HTTP methods', async () => {
    ...
    for (const method of methods) {
        const res = await app.handle(new Request('http://localhost/wildcard', { method }));
        expect(res.status).toBe(200);
        const result = await res.json();
...
test('route without method defaults to * (all methods)', async () => {
    ...
    for (const method of methods) {
        const res = await app.handle(new Request('http://localhost/default', { method }));
        expect(res.status).toBe(200);
        const result = await res.json();
        expect(result).toEqual({ method });
    }
...
test('onError fires on validation errors', async () => {
    ...
    expect(errorMessage).toMatchInlineSnapshot(`"name: Invalid input: expected string, received number"`);
...
test('validate body works, request fails', async () => {
    ...
    expect(res.status).toBe(422);
    expect(await res.text()).toMatchInlineSnapshot(`"{"code":"VALIDATION","status":422,"message":"requiredField: Invalid input: expected string, received undefined"}"`);
});
test('run use', async () => {
    const res = await new Spiceflow()
        .use(({ request }) => {
...
test('does not append subapp basePath if it is the same as parent app', async () => {
    const app = new Spiceflow({ basePath: '/api' })
        ...
        .use(new Spiceflow({ basePath: '/api' }).use(new Spiceflow({ basePath: '/v1' }).get('/posts', () => 'posts')));
    // Test that /api/users works (not /api/api/users)
    {
        const res = await app.handle(new Request('http://localhost/api/users', { method: 'GET' }));
        expect(res.status).toBe(200);
        expect(await res.json()).toEqual('users');
    }
    // Test that /api/v1/posts works (not /api/api/v1/posts)
    {
        const res = await app.handle(new Request('http://localhost/api/v1/posts', { method: 'GET' }));
        expect(res.status).toBe(200);
...
test('does not append subapp basePath if parent is prefix of subapp path', async () => {
    const app = new Spiceflow({ basePath: '/api' })
        ...
        .use(new Spiceflow({ basePath: '/api' }).use(new Spiceflow({ basePath: '/api/v2' }).get('/posts', () => 'posts')));
    // Test that /api/sub/users works (not /api/api/sub/users)
    {
        const res = await app.handle(new Request('http://localhost/api/sub/users', { method: 'GET' }));
        expect(res.status).toBe(200);
        expect(await res.json()).toEqual('users');
    }
    // Test that /api/v2/posts works (not /api/api/v2/posts)
    {
        const res = await app.handle(new Request('http://localhost/api/v2/posts', { method: 'GET' }));
        expect(res.status).toBe(200);
        expect(await res.json()).toEqual('posts');
    }
...
describe('safePath', () => {
    ...
    test('all HTTP methods are available in safePath', async () => {
        ...
        const res = await testApp.handle(new Request('http://localhost/source', { method: 'GET' }));
        expect(res.status).toBe(200);
    ...
    test('safePath works inside route handlers', async () => {
        ...
        const res = await app.handle(new Request('http://localhost/redirect-test', { method: 'POST' }));
        expect(res.status).toBe(200);
        expect(await res.json()).toBe('/target');
    });
});
test('composition with .use() works with state and onError - child app gets same state, errors caught by root', async () => {
    ...
    const successRes = await rootApp.handle(new Request('http://localhost/success', { method: 'GET' }));
    expect(successRes.status).toBe(200);
    expect(await successRes.json()).toEqual({ counter: 11 }); // 0 + 1 + 10
    // Test error case - root onError should catch child errors
    const errorRes = await rootApp.handle(new Request('http://localhost/error', { method: 'GET' }));
...
test('onError receives path parameter', async () => {
    ...
    // Test GET request
    const getRes = await app.handle(new Request('http://localhost/test/path/123?foo=bar', { method: 'GET' }));
    expect(getRes.status).toBe(500);
    expect(capturedPath).toBe('/test/path/123?foo=bar');
    expect(capturedError.message).toBe('Test error');
    // Test POST request
    const postRes = await app.handle(new Request('http://localhost/another/route', { method: 'POST' }));
...
test('error status validation', async () => {
    ...
    for (const { status, expected } of testCases) {
        ...
        const res = await app.handle(new Request('http://localhost/test', { method: 'GET' }));
...
test('route override - different methods on same path work independently', async () => {
    ...
    const postRes = await app.handle(new Request('http://localhost/test', { method: 'POST' }));
...
test('route override with .use() - parent app routes take precedence over child app routes', async () => {
    ...
    const sharedRes = await parentApp.handle(new Request('http://localhost/shared', { method: 'GET' }));
    expect(sharedRes.status).toBe(200);
    expect(await sharedRes.json()).toMatchInlineSnapshot(`"parent handler"`);
    // Parent-only route works as expected
    const parentOnlyRes = await parentApp.handle(new Request('http://localhost/parent-only', { method: 'GET' }));
    ...
    const childOnlyRes = await parentApp.handle(new Request('http://localhost/child-only', { method: 'GET' }));
...
test('route override with .use() - parent app routes always win, regardless of order', async () => {
    ...
    const res = await parentApp.handle(new Request('http://localhost/shared', { method: 'GET' }));
    expect(res.status).toBe(200);
...
test('route override with nested .use() - first matching parent route wins', async () => {
    ...
    const res = await rootApp.handle(new Request('http://localhost/test', { method: 'GET' }));
    expect(res.status).toBe(200);
...
test('route override with .use() - child routes are accessible when no parent route exists', async () => {
    const firstChildApp = new Spiceflow().get('/child1-route', () => 'first child');
    const secondChildApp = new Spiceflow()
        .get('/child2-route', () => 'second child')
        .get('/shared-child', () => 'second child shared');
    const thirdChildApp = new Spiceflow().get('/shared-child', () => 'third child shared');
    const parentApp = new Spiceflow()
    ...
    const child1Res = await parentApp.handle(new Request('http://localhost/child1-route', { method: 'GET' }));
    expect(child1Res.status).toBe(200);
    expect(await child1Res.json()).toMatchInlineSnapshot(`"first child"`);
    // Second child route works
    const child2Res = await parentApp.handle(new Request('http://localhost/child2-route', { method: 'GET' }));
    ...
    const sharedChildRes = await parentApp.handle(new Request('http://localhost/shared-child', { method: 'GET' }));
...
test('disableSuperJsonUnlessRpc is inherited by child apps', async () => {
    ...
    const regularRes = await parentApp.handle(new Request('http://localhost/date', { method: 'GET' }));
    ...
    const rpcRes = await parentApp.handle(new Request('http://localhost/date', {
        method: 'GET',
...
test('/* as not-found handler - registered first', async () => {
    ...
    const homeRes = await app.handle(new Request('http://localhost/', { method: 'GET' }));
    expect(homeRes.status).toBe(200);
    expect(await homeRes.json()).toEqual({ message: 'Home' });
    const usersRes = await app.handle(new Request('http://localhost/users', { method: 'GET' }));
    expect(usersRes.status).toBe(200);
    expect(await usersRes.json()).toEqual({ message: 'Users list' });
    const userRes = await app.handle(new Request('http://localhost/users/123', { method: 'GET' }));
    expect(userRes.status).toBe(200);
    expect(await userRes.json()).toEqual({ message: 'User', id: '123' });
    const apiRes = await app.handle(new Request('http://localhost/api/data', { method: 'POST' }));
    expect(apiRes.status).toBe(200);
    expect(await apiRes.json()).toEqual({ message: 'Data posted' });
    // Non-existent routes should be caught by /*
    const notFoundRes = await app.handle(new Request('http://localhost/non-existent', { method: 'GET' }));
    expect(notFoundRes.status).toBe(200);
    expect(await notFoundRes.json()).toEqual({ message: 'Not found', path: 'catch-all' });
    const deepNotFoundRes = await app.handle(new Request('http://localhost/some/deep/path', { method: 'GET' }));
    ...
    const wrongMethodRes = await app.handle(new Request('http://localhost/users', { method: 'DELETE' }));
...
test('/* with all methods as not-found handler', async () => {
    ...
    const getUsersRes = await app.handle(new Request('http://localhost/api/users', { method: 'GET' }));
    expect(getUsersRes.status).toBe(200);
    expect(await getUsersRes.json()).toEqual({ message: 'GET users' });
    const postUsersRes = await app.handle(new Request('http://localhost/api/users', { method: 'POST' }));
    ...
    const notFoundGetRes = await app.handle(new Request('http://localhost/not-found', { method: 'GET' }));
    ...
    const notFoundPostRes = await app.handle(new Request('http://localhost/not-found', { method: 'POST' }));
    expect(notFoundPostRes.status).toBe(200);
    expect(await notFoundPostRes.json()).toEqual({ message: 'Custom 404', method: 'any' });
    const notFoundDeleteRes = await app.handle(new Request('http://localhost/not-found', { method: 'DELETE' }));
    ...
    const wrongMethodRes = await app.handle(new Request('http://localhost/api/users', { method: 'DELETE' }));
    expect(wrongMethodRes.status).toBe(404);
});
test('/* priority - more specific routes always win', async () => {
    ...
    const exactRes = await app.handle(new Request('http://localhost/users/special/exact', { method: 'GET' }));
    expect(exactRes.status).toBe(200);
    expect(await exactRes.json()).toBe('exact-match');
    // Next most specific catch-all wins
    const specialCatchRes = await app.handle(new Request('http://localhost/users/special/something', { method: 'GET' }));
    ...
    const usersCatchRes = await app.handle(new Request('http://localhost/users/other', { method: 'GET' }));
    ...
    const generalCatchRes = await app.handle(new Request('http://localhost/something-else', { method: 'GET' }));
